...
export const ResponseCard: React.FC<Props> = ({ response, index }) => {
  ...
  return (
    <div className={`bg-mariate-panel rounded-lg p-5 border-l-4 ${getBorderColor(response.tone)} shadow-lg hover:shadow-xl transition-all duration-300 relative group flex flex-col h-full`}>
      <div className="flex justify-between items-center mb-3">
        <div className="flex items-center space-x-2">
          <span className="text-2xl">{getIcon(response.tone)}</span>
          <h3 className="font-bold text-slate-100 text-base md:text-lg">{response.tone}</h3>
        </div>
        <div className="text-[10px] md:text-xs font-mono text-slate-400 bg-slate-800 px-2 py-1 rounded border border-slate-700">
          OPTION {String.fromCharCode(65 + index)}
        </div>
      </div>
      
      <div className="bg-mariate-dark p-4 rounded border border-slate-700 mb-4 flex-grow min-h-[100px]">
        <p className="text-slate-200 text-sm md:text-base leading-relaxed whitespace-pre-wrap font-medium">
          "{response.content}"
        </p>
      </div>
